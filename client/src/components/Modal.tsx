...
export const Modal: React.FC<tailwindProps> = ({ size = 'fit', bg = 'bg-white', color, ...props }): JSX.Element => {
	return (
		<div {...props} className={`${size} ${bg} ${color} p-5 rounded-[1.25rem] ${props.className} `}>
			{props.children}
		</div>
	);
};
...
export const ModalHeader: React.FC<modalHeaderTypes> = ({ onHide, closeButton = true, ...props }): JSX.Element => {
	return (
		<div {...props} className={`${props.className}`}>
			{closeButton && (
                <ImCancelCircle className='w-5 h-5 text-text-gray-primary cursor-pointer ml-auto ' onClick={onHide}/>
			)}
		</div>
	);
};
