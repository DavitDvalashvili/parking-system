    ...
    <div className="p-10 pr-[7.625rem] flex flex-col gap-10">
	  ...
	  <div className='grid grid-cols-3 gap-10'>
				{cards.map((card: card, i: number) => {
					return <SingleCard card={card} key={i} showEditCard={() => showEditCard(card)} showRemoveCard={() => showRemoveCard(card)} />;
				})}
			</div>
			{isCardEditing && <EditCard card={selectedCard} setCards={setCards} hideEditCard={hideEditCard} />}
			{isCardRemoving && <RemoveCard card={selectedCard} setCards={setCards} hideRemoveCard={hideRemoveCard} />}
    </div>
  );
};
